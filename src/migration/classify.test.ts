...
import { classifyAll, classifyMigration } from "./classify"
...
describe("per-statement classification", () => {
  function makeSnapshot(id: string, ddl: Snapshot["ddl"] = []): Snapshot {
    return { version: "8", dialect: "postgres", id, prevIds: [], ddl, renames: [] }
  }

  function makeMigration(sql: string, snapshot: Snapshot): Migration {
    return {
      dirName: "20250101000000_test",
      dirPath: "/tmp/test",
      sql,
      snapshot,
  ...
  test("DDL targeting a table NOT in the diff is classified as manual", () => {
    // Diff adds email column to "users" table, but DDL targets "orders"
    const prev = makeSnapshot("prev", TABLE_DDL)
    const curr = makeSnapshot("curr", TABLE_WITH_EMAIL_DDL)
    const migration = makeMigration(
      'ALTER TABLE "orders" ADD COLUMN "total" numeric;',
      curr,
    )
    const result = classifyMigration(migration, { ...migration, snapshot: prev } as Migration)
    expect(result.classification).toBe("manual")
    expect(result.manualStatements).toHaveLength(1)
  })

  test("DDL targeting a table IN the diff is drizzle-managed", () => {
    const prev = makeSnapshot("prev", TABLE_DDL)
    const curr = makeSnapshot("curr", TABLE_WITH_EMAIL_DDL)
    const migration = makeMigration(
    ...
    expect(result.manualStatements).toHaveLength(0)
  })

  test("untracked DDL in a non-empty-diff migration is still manual", () => {
    const prev = makeSnapshot("prev", TABLE_DDL)
    const curr = makeSnapshot("curr", TABLE_WITH_EMAIL_DDL)
    const migration = makeMigration(
    ...
    expect(result.manualStatements).toHaveLength(1)
    expect(result.manualStatements[0]).toContain("SEQUENCE")
  })

  test("DROP INDEX matched against diff is drizzle-managed", () => {
    const indexDDL: Snapshot["ddl"] = [
      ...TABLE_WITH_EMAIL_DDL,
      {
        entityType: "indexes",
        ...
        table: "users",
        nameExplicit: true,
        columns: [{ value: "email", isExpression: false, asc: true, nullsFirst: false, opclass: null }],
        isUnique: false,
        where: null,
        with: "",
        method: "btree",
    ...
    const prev = makeSnapshot("prev", indexDDL)
    const curr = makeSnapshot("curr", TABLE_WITH_EMAIL_DDL) // index removed
    const migration = makeMigration(
  ...
  test("DROP INDEX not in diff is manual", () => {
    const prev = makeSnapshot("prev", TABLE_DDL)
    const curr = makeSnapshot("curr", TABLE_WITH_EMAIL_DDL)
    const migration = makeMigration(
      'ALTER TABLE "users" ADD COLUMN "email" text;\nDROP INDEX "nonexistent_idx";',
      curr,
    )
    const result = classifyMigration(migration, { ...migration, snapshot: prev } as Migration)
    expect(result.classification).toBe("mixed")
    expect(result.manualStatements).toHaveLength(1)
    expect(result.manualStatements[0]).toContain("DROP INDEX")
  })

  test("CREATE TYPE matched against diff enums is drizzle-managed", () => {
    const withEnum: Snapshot["ddl"] = [
      ...TABLE_DDL,
      { entityType: "enums", name: "status", schema: "public", values: ["active", "inactive"] },
    ]
    const prev = makeSnapshot("prev", TABLE_DDL)
    const curr = makeSnapshot("curr", withEnum)
    const migration = makeMigration(
